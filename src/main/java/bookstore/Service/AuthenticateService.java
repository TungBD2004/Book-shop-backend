...
public class AuthenticateService {
    ...
    public Object authenticate(LoginDTO loginDTO) {
        ...
        if(user == null) {
            throw new BookShopAuthenticationException(ErrorMessage.User.USER_NOT_EXISTS, ErrorCode.CODE_ERROR, ErrorObject.USER);
        }
        boolean authenticated = passwordEncoder.matches(loginDTO.getPassword(), user.getPassword());

        if(!authenticated) {
            throw new BookShopAuthenticationException(ErrorMessage.User.EMAIL_OR_PASSWORD_INCORRECT, ErrorCode.CODE_ERROR, ErrorObject.USER);
        }
    ...
    public Object register(RegisterDTO registerDTO) {
        ...
        if(user!= null) {
            throw new BookShopAuthenticationException(ErrorMessage.User.USER_ALREADY_EXISTS, ErrorCode.CODE_ERROR, ErrorObject.USER);
        }
        ...
        if (!registerDTO.getPassword().equals(registerDTO.getRePassword())) {
            throw new BookShopAuthenticationException(ErrorMessage.User.RE_PASSWORD_NOT_MATCH, ErrorCode.CODE_ERROR, ErrorObject.USER);
        }
